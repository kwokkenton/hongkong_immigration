...
st.write(
    """## Most frequently used control points
There are 15 control points in Hong Kong.
"""
)
df_freq_arrivals = (
    df[["control_point", "arrivals", "departures"]]
    .groupby(["control_point"])
    .sum()
...
for i in ["Hong Kong Residents", "Mainland Visitors", "Other Visitors"]:
    st.write("### " + i)
    df_aggregate_numbers = (
        df[df.identity == i].groupby(["date"]).sum()[["arrivals", "departures", "flux"]]
    )
